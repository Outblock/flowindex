        ...
        <div className="space-y-4">
            ...
            {initialNodes.length > 0 && (
                <ExpandableFlowContainer
                    label="Asset Flow"
                    subtitle={`${data?.flows?.length || 0} flow${(data?.flows?.length || 0) !== 1 ? 's' : ''} · ${initialNodes.length} address${initialNodes.length !== 1 ? 'es' : ''}`}
                    icon={<Sparkles className="w-4 h-4 text-purple-500" />}
                    height={Math.max(250, Math.ceil(initialNodes.length / 2) * 100 + 80)}
                >
                    <FlowDiagram
