...
import { fetchAccountStorage, fetchAccountStorageLinks, fetchAccountStorageItem } from '../../api/heyapi';
...
export function AccountStorageTab({ address }: Props) {
    ...
    const loadOverview = async () => {
        ...
        try {
            const [storageRes, linksRes] = await Promise.all([
                fetchAccountStorage(normalizedAddress),
                fetchAccountStorageLinks(normalizedAddress, 'public'),
            ]);

            const storageDecoded: any = decodeCadenceValue(storageRes);
            const linksDecoded: any = decodeCadenceValue(linksRes);
    ...
    const browseStoragePath = async (path: string) => {
        ...
        try {
            ...
            const raw: any = await fetchAccountStorageItem(normalizedAddress, identifier);
            setRawItem(raw);
