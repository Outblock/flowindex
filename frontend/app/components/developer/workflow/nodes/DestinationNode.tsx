...
function DestinationNode({ data, selected }: NodeProps) {
  ...
  const target = config.url || config.webhook_url || config.chat_id || config.to || ''
  const tmpl = config.message_template || ''

  return (
    <div
      className={`bg-white dark:bg-neutral-800 border rounded-xl px-4 py-3 min-w-[160px] max-w-[220px] transition-shadow ${
    ...
    >
      ...
      {target && (
        <p className="text-xs text-zinc-500 dark:text-neutral-400 truncate">{String(target)}</p>
      )}
      {tmpl && (
        <p className="text-[10px] text-zinc-400 dark:text-neutral-500 truncate mt-0.5 italic">{String(tmpl)}</p>
      )}
    </div>
  )
}
