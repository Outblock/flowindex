...
export class SolidityLSPClient extends EventEmitter {
  ...
  private async _initialize(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      this.process!.on('error', (err) => reject(new Error(`Failed to start Solidity LSP: ${err.message}`)));
  ...
  async request(method: string, params: any, timeoutMs = 30000): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Solidity LSP request '${method}' timed out after ${timeoutMs}ms`));
      }, timeoutMs);
