...
export class PasskeySigner implements FlowSigner {
  ...
  constructor(config: SignerConfig, options?: PasskeySignerOptions) {
    this.config = config;
    if (options?.flowAddress) this.flowAddress = options.flowAddress;
    if (options?.keyIndex !== undefined) this.keyIndex = options.keyIndex;
  ...
  async init(): Promise<void> {
    ...
    // approval flow. Address/keyIndex/token can be set via constructor options.
  }
