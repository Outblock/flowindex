...
type HistoryDeriver struct {
	...
	// Parameterized checkpoint names (derived from CheckpointPrefix).
	upCheckpoint   string
	downCheckpoint string
	logPrefix      string
	// Directional disable flags.
	disableUp   bool
	disableDown bool
	// Hard upper bound for UP scan (0 = use worker floor).
...
type HistoryDeriverConfig struct {
	...
	CheckpointPrefix   string // checkpoint name prefix (default: "history_deriver")
	DisableUp          bool   // skip processUpward entirely
	DisableDown        bool   // skip processDownward entirely
	CeilingHeight      uint64 // hard upper bound for UP (0 = use worker floor)
}
...
func NewHistoryDeriver(repo *repository.Repository, processors []Processor, cfg HistoryDeriverConfig) *HistoryDeriver {
	...
	if prefix == "" {
		prefix = "history_deriver"
	}
	upCkpt := prefix
	downCkpt := prefix + "_down"
	logPrefix := "[" + prefix + "]"
	return &HistoryDeriver{
		...
		upCheckpoint:       upCkpt,
		downCheckpoint:     downCkpt,
		logPrefix:          logPrefix,
		disableUp:          cfg.DisableUp,
		disableDown:        cfg.DisableDown,
...
func (h *HistoryDeriver) Start(ctx context.Context) {
	if len(h.processors) == 0 {
		log.Printf("%s Disabled: no processors configured", h.logPrefix)
		return
	}
	log.Printf(
		"%s Starting (processors=%d chunk=%d concurrency=%d timeout_ms=%d disableUp=%v disableDown=%v ceiling=%d)",
		h.logPrefix,
		len(h.processors),
		h.chunkSize,
		h.concurrency,
...
func (h *HistoryDeriver) run(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			log.Printf("%s Error: %v", h.logPrefix, err)
			select {
			case <-ctx.Done():
				return
			case <-time.After(10 * time.Second):
...
func (h *HistoryDeriver) processUpward(ctx context.Context) (bool, error) {
	upCursor, err := h.repo.GetLastIndexedHeight(ctx, h.upCheckpoint)
	if err != nil {
		return false, err
	}
	...
	// Clamp ceiling to hard upper bound if configured.
	if h.ceilingHeight > 0 && h.ceilingHeight < ceiling {
		ceiling = h.ceilingHeight
	}
	...
	if !hasBlocks {
		if scanFrom%100000 < h.chunkSize {
			log.Printf("%s UP: no raw blocks in [%d,%d), waiting for backward ingester", h.logPrefix, chunks[0].from, chunks[0].to)
		}
		return false, nil
	}
	...
	for i, c := range chunks {
		if results[i] != nil {
			log.Printf("%s UP: chunk [%d,%d) failed: %v — stopping at %d", h.logPrefix, c.from, c.to, results[i], advanceTo)
			break
		}
		advanceTo = c.to
	}
	...
	if advanceTo > scanFrom {
		if err := h.repo.UpdateCheckpoint(ctx, h.upCheckpoint, advanceTo); err != nil {
			log.Printf("%s Failed to update up checkpoint: %v", h.logPrefix, err)
		}
		if advanceTo%100000 < h.chunkSize*uint64(h.concurrency) {
			log.Printf("%s Progress UP: processed to %d (ceiling=%d, chunks=%d)", h.logPrefix, advanceTo, ceiling, len(chunks))
		}
		return true, nil
	}
...
func (h *HistoryDeriver) processDownward(ctx context.Context) (bool, error) {
	downCursor, err := h.repo.GetLastIndexedHeight(ctx, h.downCheckpoint)
	if err != nil {
		return false, err
	}
	...
	if downCursor == 0 {
		...
		upCursor, _ := h.repo.GetLastIndexedHeight(ctx, h.upCheckpoint)
		if upCursor == 0 {
	...
	for i, c := range chunks {
		if results[i] != nil {
			log.Printf("%s DOWN: chunk [%d,%d) failed: %v — stopping at %d", h.logPrefix, c.from, c.to, results[i], advanceTo)
			break
		}
		advanceTo = c.from
	}
	...
	if advanceTo < downCursor {
		if err := h.repo.UpdateCheckpointDown(ctx, h.downCheckpoint, advanceTo); err != nil {
			log.Printf("%s Failed to update down checkpoint: %v", h.logPrefix, err)
		}

		if advanceTo%100000 < h.chunkSize*uint64(h.concurrency) || advanceTo == minRaw {
			log.Printf("%s Progress DOWN: cursor %d -> %d (minRaw=%d, chunks=%d)", h.logPrefix, downCursor, advanceTo, minRaw, len(chunks))
		}
		return true, nil
	}
			...
			go func(proc Processor) {
				...
				for attempt := 1; attempt <= maxDeadlockRetries; attempt++ {
					...
					if attempt < maxDeadlockRetries {
						log.Printf("%s %s range [%d,%d) deadlock (attempt %d/%d), retrying...", h.logPrefix,
				...
				if lastErr != nil {
					log.Printf("%s %s range [%d,%d) failed: %v", h.logPrefix, proc.Name(), from, to, lastErr)
				...
				if dur := time.Since(began); dur > 2*time.Second {
					log.Printf("%s %s range [%d,%d) took %s", h.logPrefix, proc.Name(), from, to, dur)
				}
			}(p)
		}
		wg.Wait()
