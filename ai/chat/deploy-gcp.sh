...
# Database connections (set via environment or .env file)
FLOWINDEX_DB_URL="${FLOWINDEX_DATABASE_URL:-${DATABASE_URL:-}}"
BLOCKSCOUT_DB_URL="${BLOCKSCOUT_DATABASE_URL:-}"

[[ -n "${FLOWINDEX_DB_URL}" ]] || err "FLOWINDEX_DATABASE_URL or DATABASE_URL must be set"
...
sudo docker run -d \
  ...
  -p 8084:8084 \
  -p 3001:3001 \
  -e DATABASE_URL='${FLOWINDEX_DB_URL}' \
  -e FLOWINDEX_DATABASE_URL='${FLOWINDEX_DB_URL}' \
  -e BLOCKSCOUT_DATABASE_URL='${BLOCKSCOUT_DB_URL}' \
