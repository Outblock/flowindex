...
export function flowIndexWallet(
  ...
  return function createWallet(_params: any) {
    return {
      ...
      createConnector(_walletDetails: any) {
        // This must return a CreateConnectorFn compatible with wagmi
        // We dynamically import createConnector from wagmi at the dApp level
        return createFlowIndexConnector({ walletUrl })
      },
    }
  }
}
...
/**
 * Creates a wagmi-compatible connector function.
 * Uses the same shape as wagmi's createConnector return value.
 */
function createFlowIndexConnector(config: FlowIndexProviderConfig) {
  // Return a CreateConnectorFn — wagmi calls this with its Config
  return function setup(wagmiConfig: any) {
